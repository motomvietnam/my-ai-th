...
with tabs[2]:
    ...
    with col_dl1:
        st.download_button(
            ...
            data=tạo_excel_mẫu(),
            file_name="1_Mau_Excel_Data.xlsx",
    ...
    with col_dl3:
        st.download_button(
            ...
            file_name="3_Mau_Hop_Dong.docx",
            use_container_width=True
        )

    st.divider()

    # 2. KHU VỰC TẢI LÊN VÀ NHẬP LIỆU
    st.subheader("🚀 Bước 2: Tải file Word của bạn & Nhập dữ liệu")
    c1, c2 = st.columns(2)
    with c1:
        word_template = st.file_uploader("📂 Tải file Word mẫu (Chứa {{ từ khóa }})", type=["docx"], key="w_tpl_v4")
    with c2:
        st.info("💡 **Mẹo:** Bạn có thể dán (Ctrl+V) dữ liệu trực tiếp từ Excel vào bảng phía dưới để xử lý nhanh.")

    # Cấu hình các cột hiển thị đậm và rõ nét
    cột_yêu_cầu = [
        "So", "Ten", "ChucVu", "Luong", "TenKhach", "TenSuKien", 
        "ThoiGian", "DiaDiem", "NgayCap", "LuongMoi", "LuongCu", 
        "NgayHieuLuc", "MaNV", "Phongban"
    ...
    # Bảng nhập liệu với định dạng chữ đen đậm
    st.write("📝 **Bảng dữ liệu trộn (Chữ đen đậm):**")
    edited_df = st.data_editor(
        st.session_state.df_merge, 
        num_rows="dynamic", 
        use_container_width=True,
        column_config={col: st.column_config.TextColumn(label=f"**{col}**") for col in cột_yêu_cầu},
        key="pro_editor_final"
    )

    # 3. XỬ LÝ TRỘN FILE VÀ XUẤT ZIP
    if st.button("🚀 BẮT ĐẦU XUẤT HÀNG LOẠT (.ZIP)", use_container_width=True):
        if word_template and not edited_df.empty:
            try:
                ...
                with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
                    for index, row in edited_df.iterrows():
                        ...
                        # Tự động chuyển số tiền thành chữ cho các trường lương
                        ...
                        if "Luong" in context and context["Luong"]:
                            context["LuongChu"] = doc_so_thanh_chu_logic(context["Luong"])
                        
                        # Render dữ liệu vào Word
                        doc.render(context)
                        ...
                        # Đặt tên file (Ưu tiên cột Ten, sau đó tới So)
                        ten_file = str(row.get('Ten', row.get('So', f'Ho_So_{index+1}'))).strip().replace(' ', '_')
                        zip_file.writestr(f"{ten_file}.docx", out_word.getvalue())
                
                st.success(f"🎉 Chúc mừng! Đã tạo xong {len(edited_df)} tài liệu.")
                st.download_button(
            ...
            st.warning("⚠️ Vui lòng tải file Word mẫu lên và nhập ít nhất một dòng dữ liệu vào bảng!")
